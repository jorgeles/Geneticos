...
public class Main {
	...
	public static void main(String[] args) {
		...
		// -------------------------------------------------------------
		/*
		 * Configuration conf = new DefaultConfiguration(); try {
		 * conf.setRandomGenerator(randomGenerator); } catch
		 * (InvalidConfigurationException e2) { // TODO Auto-generated catch
		...
		// ------------------------------------------------------------
		/*
		 * int targetAmount = Integer.parseInt("20"); FitnessFunction myFunc =
		 * new MinimizingMakeChangeFitnessFunction( targetAmount);
		 * 
		 * try { conf.setFitnessFunction(myFunc); } catch
		 * (InvalidConfigurationException e1) { // TODO Auto-generated catch
		 * block e1.printStackTrace(); }
		 * 
		 * // Now we need to tell the Configuration object how we want our //
		 * Chromosomes to be setup. We do that by actually creating a // sample
		 * Chromosome and then setting it on the Configuration // object. As
		 * mentioned earlier, we want our Chromosomes to // each have four
		 * genes, one for each of the coin types. We // want the values of those
		 * genes to be integers, which represent // how many coins of that type
		 * we have. We therefore use the // IntegerGene class to represent each
		 * of the genes. That class // also lets us specify a lower and upper
		 * bound, which we set // to sensible values for each coin type. //
		 * -------------------------------------------------------------- Gene[]
		 * sampleGenes = new Gene[4];
		 * 
		 * try { sampleGenes[0] = new IntegerGene(conf, 0, 4); sampleGenes[1] =
		 * new IntegerGene(conf, 0, 4); // Dimes sampleGenes[2] = new
		 * IntegerGene(conf, 0, 4); // Nickels sampleGenes[3] = new
		 * IntegerGene(conf, 0, 4); // Pennies
		 * 
		 * 
		 ...
		 * Chromosomes we want in our population. The more Chromosomes, // the
		 * larger the number of potential solutions (which is good // for
		 * finding the answer), but the longer it will take to evolve // the
		 * population each round. We'll set the population size to // 500 here.
		 * // --------------------------------------------------------------
		 * conf.setPopulationSize(500); Genotype population =
		 * Genotype.randomInitialGenotype( conf ); IChromosome bestSolutionSoFar
		 ...
		 * .amountOfChange(bestSolutionSoFar) + " cents in " +
		 * MinimizingMakeChangeFitnessFunction
		 * .getTotalNumberOfCoins(bestSolutionSoFar) + " coins.");
		 * 
		 * } catch (InvalidConfigurationException e) { // TODO Auto-generated
		 * catch block e.printStackTrace(); } // Quarters
		 */
